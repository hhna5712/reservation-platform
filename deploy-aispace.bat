...
echo 📦 배포 파일 생성 중...
tar -czf ..\reservation-platform-deploy.tar.gz --exclude=node_modules --exclude=.next --exclude=.git .
...
echo.
echo 3. 파일 업로드:
echo    %cd%\..\reservation-platform-deploy.tar.gz
echo.
echo 4. 환경변수 설정:
